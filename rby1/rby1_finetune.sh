...
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-392}"
...
LETTER_BOX_TRANSFORM="${LETTER_BOX_TRANSFORM:-1}"
...
CROP_FRACTION="${CROP_FRACTION:-1.0}" # Do not change this!
EXTRA_ARGS=()

usage() {
    cat <<'EOF'
Usage: bash rby1/rby1_finetune.sh \
...
if [ "$LETTER_BOX_TRANSFORM" = "1" ]; then
    LAUNCH_CMD+=(--letter-box-transform)
else
    LAUNCH_CMD+=(--no-letter-box-transform)
fi
